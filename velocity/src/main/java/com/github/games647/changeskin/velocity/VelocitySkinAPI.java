...
public class VelocitySkinAPI implements ChangeSkinAPI<Player, GameProfile> {
    ...
    private final ChangeSkinVelocity plugin;

    public VelocitySkinAPI(ChangeSkinVelocity plugin) {
        this.plugin = plugin;
    }

    @Override
    public void applySkin(Player player, SkinModel targetSkin) {
        plugin.getLog().debug("Applying skin for {}", player.getUsername());
        ...
        if (loginProfile == null) {
            ...
            loginProfile = new GameProfile(mojangUUID, player.getUsername(), toProperties(targetSkin));
